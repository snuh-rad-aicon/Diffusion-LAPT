...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 original_img_weight=1.0,
                 original_lesion_weight=10.0,
        ...
        self.original_img_weight = original_img_weight
        self.original_lesion_weight = original_lesion_weight
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        lesion = super().get_input(batch, 'lesion')
        out = [z, c, mask, lesion]
    ...
    def shared_step(self, batch, **kwargs):
        ...
        # input {'mask': mask, 'lesion': lesion} as kwargs
    ...
    def p_losses(self, x_start, cond, t, noise=None, mask=None, lesion=None):
        ...
        # # type 1: noise image (at t) using q_sample
        # if self.parameterization == "eps":
        #     x_noisy_pred = self.q_sample(x_start=x_noisy, t=t, noise=model_output)
        # elif self.parameterization == "x0":
        #     x_noisy_pred = self.q_sample(x_start=model_output, t=t, noise=noise)
        # else:
        #     raise NotImplementedError()
        # img_pred = self.differentiable_decode_first_stage(x_noisy_pred)
        # target_img = self.differentiable_decode_first_stage(x_noisy)
        
        
        # type 2: clean image (at 0)
        if self.parameterization == "eps":
            ...
            raise NotImplementedError()
        # decoder for image space
        img_pred = self.differentiable_decode_first_stage(x_start_pred)
        target_img = self.differentiable_decode_first_stage(x_start)
        
        # calculate mse loss between img_pred and target_img and weighted by mask and lesion
        loss_img = self.get_loss(img_pred, target_img, mean=False)
        
        loss_image = (loss_img).sum(dim=(1, 2, 3))
        loss_image = (self.img_weights[t] * loss_image).sum()
        loss_image = loss_image / mask.sum()
        loss_dict.update({f'{prefix}/loss_image': loss_image})
        loss += (self.original_img_weight * loss_image)
        
        if self.original_lesion_weight > 0:
            loss_lesion = (loss_img * lesion).sum(dim=(1, 2, 3))
            loss_lesion = (self.lesion_weights[t] * loss_lesion).sum()
            loss_lesion = loss_lesion / lesion.sum()
            loss_dict.update({f'{prefix}/loss_lesion': loss_lesion})
            loss += (self.original_lesion_weight * loss_lesion)
